...
\end{equation}
where $\ket{q_{\perp}}=-\ket{\alpha}\braket{\beta|q}+\ket{\beta}\braket{\alpha|q}$, we wish $j=\frac{\Delta\varphi}{\omega}$ is an integer, so that we have $Q^{j}\ket{\psi}=\ket{\beta}$.
