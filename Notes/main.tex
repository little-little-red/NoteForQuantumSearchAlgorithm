...
\begin{theorem}
    Cosidering vectors $\vec{m},\vec{n}\in\mathbb{R}^{3}$ and angle $\phi\in[0,2\pi]$, for a certainly axis defined by vector $\vec{l}$, the inner product $\vec{m}\cdot\left(\exp\left(\phi\vec{l}\cdot\vec{J}\right)\vec{n}\right)$ take the extreme only when $\det\left\lvert\left(\vec{m},\vec{n},\vec{l}\right)\right\rvert<3$, which $J_{j}$ are the three generators of Lie group $SO(3)$.
\end{theorem}
