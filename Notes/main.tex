...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    where we all know that $(\vec{n}\cdot\vec{\sigma})^{2} = I$.
\end{proof}
\begin{theorem}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{split}
    \end{equation*}
    where the last equality follows from Theorem \ref{ProductOfPauli}.
\end{proof}
...
\begin{theorem}
    ...
    \end{equation}
\end{theorem}
In the case of euclidean distances, we conclude that Grover's algorithm is asymptotically optimal, and in the case of  angular distances, that Grover's algorithm is exactly optimal.\footnote{This quote is from the original paper\cite{2009Exact}. I did not understand the complexity of other algorithms, so I put it here without modification.}
