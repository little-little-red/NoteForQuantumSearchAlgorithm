...
\chapter{Bounds for Grover's algorithm\label{sec:BoundForGrover}}

\textit{This chapter is based on the paper\cite{2009Exact}.}
...
\end{equation}
So that we can find the distance changed by oracle, here we use two kinds of metric, euclidean distance
\begin{equation}
...
\section{\label{DistanceByOracle}Distance the oracle changes}
...
\section{\label{DistanceWhenWeNeed}Distance when the output is we need}
...
\section{Lower bound of Grover iterations}
...
\chapter{Long's algorithm}

\textit{This section is based on the paper\cite{Long_2001}.}

\section{\label{subsec:LongMainIdea}Main idea}
...
\begin{figure}[h]
    \centering
    \resizebox{0.45\textwidth}{!}{\input{figures/Long.pgf}}
    \caption{}
    \label{fig:long}
\end{figure}
...
\section{The right angle\label{subsec:RightAngle}}
...
It can be caculated that, as $N$ increases, the angle $\phi$ approaches $\pi$. At this point, Long's algorithm is also slightly closer to Grover's algorithm.



\chapter{Abdulrahman's work}
...
\section{Keep states in the plane}
...
\end{equation}
Next, let's consider the effect of the Grover iteration $G$ on the angle $\varphi$, what can be computed is that the Oracle gives us the mapping for states in the plane
\begin{equation}
    \begin{split}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Since the plane gives us the angle $\varphi=\pi/4$, we need to operate $A\coloneqq A(-2(\varphi-(-\varphi)))=A(-\pi)$ to pull back the angle $\varphi$ to $\pi/4$, so the operation $Q$ gives us the mapping for states in the plane
\begin{equation}
    \begin{split}
