...
\subsubsection{Properties of single-qubit gate}
...
\begin{proof}
    ...
    \begin{equation}\label{EulerAngleProof}
        U=e^{i\alpha}
        \begin{pmatrix}
        ...
        \end{pmatrix}
    \end{equation}
    Equation \ref{EulerAngleProof} now follows immediately from the definition of the rotation matrices and matrix multiplication.
\end{proof}
\begin{theorem}\label{DecToAXBXC}
...
\subsection{Bounds for Grover's algorithm}
...
\subsubsection{\label{DistanceByOracle}Distance the oracle changes}
...
\subsubsection{\label{DistanceWhenWeNeed}Distance when the output is we need}
...
\subsubsection{Lower bound of grover iterations}
