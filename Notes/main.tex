...
\begin{equation}
    ...
    \end{split}
\end{equation}
which $J_{j}$ is the three generators of Lie group $SO(3)$, the former refers to single-qubit gates and the latter to transformations on the Bloch sphere. For visualization and convenience, we let $R_{\vec{n}}(\theta) \coloneqq \exp\left(-i\frac{\theta}{2}\vec{n}\cdot\vec{\sigma}\right)$.
...
\begin{corollary}
    ...
    \begin{equation}
        R_{\vec{n}}(\theta) \coloneqq \exp\left(-i\frac{\theta}{2}\vec{n}\cdot\vec{\sigma}\right)=\cos \left(\frac{\theta}{2}\right)I-i\sin \left(\frac{\theta}{2}\right)(\vec{n}\cdot\vec{\sigma}).
    \end{equation}
\end{corollary}
\begin{proof}
    ...
    \begin{equation}
        \begin{split}
            \exp\left(-i\frac{\theta}{2}\vec{n}\cdot\vec{\sigma}\right)
             & = \sum_{k=0}^{\infty}\frac{1}{k!}\left(-i\frac{\theta}{2}\vec{n}\cdot\vec{\sigma}\right)^{k}                                                                                                                          \\
             & = \sum_{k=0}^{\infty}\frac{1}{(2k)!}\left(-i\frac{\theta}{2}\right)^{2k}(\vec{n}\cdot\vec{\sigma})^{2k} + \sum_{k=0}^{\infty}\frac{1}{(2k+1)!}\left(-i\frac{\theta}{2}\right)^{2k+1}(\vec{n}\cdot\vec{\sigma})^{2k+1} \\
...
\end{proof}
\begin{corollary}
    If a rotation through an angle $2\theta_{1}$ about an axis $\vec{n_{1}}$ is followed by a rotation through an angle $2\theta_{2}$ about an axis $\vec{n_{2}}$, then the overall rotation is through an angle $\theta_{12}$ about an axis $\vec{n_{12}}$ given by
    \begin{equation*}
        \begin{split}
            \cos\theta_{12}             & = \cos\theta_{1}\cos\theta_{2}-\sin\theta_{1}\sin\theta_{2}\vec{n_{1}}\cdot\vec{n_{2}}                                                      \\
            \sin\theta_{12}\vec{n_{12}} & = \sin\theta_{1}\cos\theta_{2}\vec{n_{2}}+\cos\theta_{1}\sin\theta_{2}\vec{n_{2}}+\sin\theta_{1}\sin\theta_{2}\vec{n_{1}}\times\vec{n_{2}}.
        \end{split}
    \end{equation*}
\end{corollary}
\begin{proof}
    We know $R_{\vec{n_{1}}}(2\theta_{1})=\cos\theta I-i\sin\theta(\vec{n_{1}}\cdot\vec{\sigma})$ and the same as index $2$, so the product of two rotations is
    \begin{equation*}
        \begin{split}
              & R_{\vec{n_{1}}}(2\theta_{1})R_{\vec{n_{2}}}(2\theta_{2})                                                                                                                                                                                       \\
            = & \cos\theta_{1}\cos\theta_{2}I-i\sin\theta_{1}\cos\theta_{2}(\vec{n_{1}}\cdot\vec{\sigma})-i\cos\theta_{1}\sin\theta_{2}(\vec{n_{2}}\cdot\vec{\sigma})-\sin\theta_{1}\sin\theta_{2}(\vec{n_{1}}\cdot\vec{\sigma})(\vec{n_{2}}\cdot\vec{\sigma}) \\
            = & \cos\theta_{12}I-i\sin\theta_{12}(\vec{n_{12}}\cdot\vec{\sigma}),
        \end{split}
    \end{equation*}
...
From the decomposition given by corollary \ref{cor:DecompositionByTwoAxis}, we can further care about the degree of approximation of $R_{\vec{n}}(\theta)$. Let's start by introducing a neat lemma :
...
\begin{equation}
    ...
    \end{split}
\end{equation}
According the proof, we just need to find the gate that has the Angle that the theorem requires. Fortunately, $THTH$ gate has the angel we need. It is a rotation of the Bloch sphere about an axis along $\vec{n} = (\cos\frac{\pi}{8},\sin\frac{\pi}{8},\cos\frac{\pi}{8})$ and through an angle $\theta$ defined by $\cos\frac{\theta}{2} = \cos^{2}\frac{\pi}{8}$, so we just let $R_{\vec{n}}(\theta) = THTH$.
...
\begin{equation}\label{FinalApproximation}
    E(U,\prod_{j=1}^{2k}R_{\vec{n}}(\theta)^{j_{n}}H) \leq k\epsilon
\end{equation}
\begin{theorem}
