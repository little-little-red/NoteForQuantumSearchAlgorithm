...
\begin{equation}\label{BlochSphereRepersentation}
    ...
    \end{split}
\end{equation}
where $\theta\in\left[0,\pi\right]$ and $\psi\in\left[0,2\pi\right]$.
...
\subsection{\label{SingleAndCnot}Single-qubit and CNOT gates are universal}
...
From corollary \ref{cor:DecompositionByTwoAxis}, to construct unitary operator $U$, we still need to approximate the rotation of the other axis, but even more fortunate is that $HR_{\vec{n}}(\omega)H$ is exactly what we need, has axis $\vec{m} = (\cos\frac{\pi}{8},-\sin\frac{\pi}{8},\cos\frac{\pi}{8})$ and the Angle, like $R_{\vec{n}}(\omega)$, is an irrational multiple of $\pi$. After layers and layers of preparation, let's list the final approximation with suitable positive integers $j_{n}$ to U:
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
such that $U_{\zeta}$ satisfies the properties we need:
\begin{equation}
    \begin{split}
        U_{\zeta}\ket{\beta}  & =-\ket{\beta}  \\
        U_{\zeta}\ket{\alpha} & =\ket{\alpha}.
    \end{split}
\end{equation}
So the $U_{\zeta}$ is the Oracle.
...
\begin{equation}
    G=WO=(2\ket{\psi}\bra{\psi}-I)U_{\zeta}.
\end{equation}
...
In the grover's algorithm we construct rotations in the space spanned by $\ket{\alpha}$ and $\ket{\beta}$, but $SU(2)$ has three degrees of freedom, so we may as well extend $G=WO=(I-2\ket{\psi_{\perp}}\bra{\psi_{\perp}})(I-2\ket{\beta}\bra{\beta})$ to
\begin{equation}
    \begin{split}
