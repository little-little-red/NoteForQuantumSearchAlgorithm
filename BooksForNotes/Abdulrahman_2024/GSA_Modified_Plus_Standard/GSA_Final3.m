...
MaxItr              = floor(pi/4*sqrt(N))*1;       % Max. number of iterations
%%==========================Initialization=================================
...
for Mthd = 1:2                 % Comparison between the standard and modified versions
    for k = 1:MaxItr
        ...
        if Mthd == 1           % Standard Grover Search Algorithm
            theta(k)         = 0;
            R                = [cos(theta(k)/2) -sin(theta(k)/2); sin(theta(k)/2) cos(theta(k)/2)] * Z; % *Z
        elseif Mthd == 2       % Modifed Grover Search Algorithm
            theta(k)         = 2*atan(1-4/N);
            if k==1
                R            = [cos(theta(k)/2) -sin(theta(k)/2); sin(theta(k)/2) cos(theta(k)/2)] * Z; % *Z
            else
                R            = [cos(theta(k)/2) -sin(theta(k)/2); sin(theta(k)/2) cos(theta(k)/2)] * H; % *H (Z+X)/sqrt(2)
            end
        end
        % Contribution of the paper (generating new rotation-around-y-axis gate)
